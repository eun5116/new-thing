...
ROOT = Path(__file__).resolve().parents[1] / "weekly_market_report"
...
class KOSPIAlertFallbackTests(unittest.TestCase):
    def test_resolve_alert_universe_uses_cached_kospi_symbols_when_live_fetch_fails(self):
        ...
        with patch.object(weekly_report, "_is_offline_mode", return_value=False), patch.object(
            weekly_report, "_get_latest_kospi_date", side_effect=RuntimeError("krx unavailable")
        ), patch.object(weekly_report, "_get_cached_kospi_symbols", return_value=["005930", "000660"]):
    ...
    def test_resolve_alert_universe_uses_default_kospi20_when_cache_is_empty(self):
        ...
        with patch.object(weekly_report, "_is_offline_mode", return_value=False), patch.object(
            weekly_report, "_get_latest_kospi_date", side_effect=RuntimeError("krx unavailable")
        ), patch.object(weekly_report, "_get_cached_kospi_symbols", return_value=[]):
            symbols, metadata = weekly_report._resolve_alert_universe(config)

        self.assertEqual(symbols, [code for _, code in weekly_report.DEFAULT_KOSPI20])
        self.assertEqual(metadata, {code: name for name, code in weekly_report.DEFAULT_KOSPI20})

    def test_render_report_shows_nearest_momentum_candidates_when_none_triggered(self):
        config_path = Path(__file__).resolve().parents[1] / "configs" / "dataset" / "kospi_alerts.yaml"
        ...
        _, text, html = weekly_report.render_report(
    ...
    def test_two_week_momentum_triggers_short_term_overheat_signal(self):
        config = weekly_report.load_alert_config(
            Path(__file__).resolve().parents[1] / "configs" / "dataset" / "kospi_alerts.yaml"
        )
        series = pd.Series(
        ...
        result = weekly_report.evaluate_two_week_momentum(series, config)
