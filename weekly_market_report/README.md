...
- Local artifact save (`outputs/*.txt`, `outputs/*.html`, `outputs/*.pdf`, `outputs/*.json`)
- PDF attachment for NotebookLM-style archiving
...
- Output inspection:
  최근 생성된 `outputs/*.json`에서 `delivery_status`와 저장된 HTML/TXT/PDF 경로를 확인할 수 있다.
- History inspection:
